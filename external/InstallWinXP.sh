...
Install Chandler on Windows XP.
 ...
 *) echo "$me: too many arguments$help" >&2
    ...
    --debug | --de* | -d )
	  ...
	  if ! test -f debug-$VERSION.tar.gz; then 
		...
		curl -O http://builds.o11n.org/external/windows/debug-$VERSION.tar.gz
	  fi
		echo "Expanding tarball"
		gunzip debug-$VERSION.tar.gz
		tar -C osaf/chandler/ -xf debug-$VERSION.tar
		dashd="-d"
	;;
    --release | --re* | -r )
	  ...
	  if ! test -f release-$VERSION.tar.gz; then 
		...
		curl -O http://builds.o11n.org/external/windows/release-$VERSION.tar.gz
	  fi
		echo "Expanding tarball"
		gunzip release-$VERSION.tar.gz
		tar  -C osaf/chandler/ -xf release-$VERSION.tar
		dashd=
	;;
    --help | --h* | -h )
       echo "$usage"; exit 0 ;;

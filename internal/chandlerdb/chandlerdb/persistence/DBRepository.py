...
from chandlerdb.persistence.Repository import \
...
from chandlerdb.persistence.DBRepositoryView import DBRepositoryView
from chandlerdb.persistence.DBContainer import \
    RefContainer, NamesContainer, ACLContainer, IndexesContainer, \
    ItemContainer, ValueContainer, VersionContainer, CommitsContainer
from chandlerdb.persistence.FileContainer import LOBContainer
from chandlerdb.persistence.LuceneContainer import IndexContainer
from chandlerdb.persistence.DBItemIO import \
...
openRepositories=set()
def closeRepositories():
    if openRepositories:
        try:
            for repo in openRepositories:
                try:
                    repo.close()
                except:
                    pass
...
atexit.register(closeRepositories)
...
class DBRepository(OnDemandRepository):
    ...
    def close(self):
        ...
        if (status & Repository.CLOSED) == 0:
            ...
            sys.modules.setdefault('chandlerdb.persistence', None)
            ...
            if self in openRepositories:
                openRepositories.remove(self)
...
class DBStore(Store):
    ...
    def getViewStatus(self, version):
        ...
        return self._versions.getViewStatus(version)

    def getViewTimezone(self, version):

        return self._versions.getViewTimezone(version)

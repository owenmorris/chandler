...
if [ ! "$CHANDLER_UNIT_TEST" = "no" ]; then
    for mode in $MODES ; do
        ...
        if [ ! "$CHANDLER_FUNCTIONAL_TEST" = "no" ]; then
            ...
            cd $C_DIR
            $CHANDLERBIN/$mode/$RUN_CHANDLER --create --profileDir="$P_DIR" --parcelPath="$PP_DIR" --scriptTimeout=600 --scriptFile="$TESTNAME" &> $T_DIR/test.log
...
if [ "$CHANDLER_PERFORMANCE_TEST" = "yes" ]; then
    ...
    for test in $TESTS ; do
        ...
        if [ `echo $test | grep -v PerfLargeData` ]; then
	        ...
	        for run in $RUNS ; do 
	            ...
	            $CHANDLERBIN/release/$RUN_CHANDLER --create --profileDir="$P_DIR" --catsPerfLog="$T_LOG" --scriptTimeout=600 --scriptFile="$TESTNAME" &> $T_DIR/test$run.log
    ...
    cd $C_DIR
    $CHANDLERBIN/release/$RUN_CHANDLER --create --profileDir="$P_DIR" --scriptTimeout=600 --scriptFile="$BACKUP_REPO" &> $C_DIR/test.log
        ...
        for test in $TESTS ; do
            ...
            if [ `echo $test | grep PerfLargeData` ]; then
		        ...
		        for run in $RUNS ; do 
		            ...
		            $CHANDLERBIN/release/$RUN_CHANDLER --restore="$REPO" --profileDir="$P_DIR" --catsPerfLog="$T_LOG" --scriptTimeout=600 --scriptFile="$TESTNAME" &> $T_DIR/test$run.log
		            echo `<"$T_LOG"` | tee -a $BUILDLOG
		        done
    ...
    $CHANDLERBIN/release/$RUN_CHANDLER --create --profileDir="$P_DIR" --scriptTimeout=600 --scriptFile="$CREATEREPO" &> $T_DIR/test.log
    
    echo Timing startup | tee -a $BUILDLOG
    for run in $RUNS ; do
        $TIME -o $T_DIR/start1.$run.log $CHANDLERBIN/release/$RUN_CHANDLER --profileDir="$P_DIR" --scriptTimeout=600 --scriptFile="$TESTNAME" &> $T_DIR/test.log
    ...
    $CHANDLERBIN/release/$RUN_CHANDLER --restore="$REPO" --profileDir="$P_DIR" --scriptTimeout=600 --scriptFile="$CREATEREPO" &> $T_DIR/test.log
    
    echo Timing startup | tee -a $BUILDLOG
    for run in $RUNS ; do
        $TIME -o $T_DIR/start6.$run.log $CHANDLERBIN/release/$RUN_CHANDLER --profileDir="$P_DIR" --scriptTimeout=600 --scriptFile="$TESTNAME" &> $T_DIR/test.log

...
LICENSE_KEY = "094WV342P99G3712NWG2"
...
def buildURLNew(search_type, keyword, product_line, type, page, license_key, locale, associate,otherArgs):
    _checkLocaleSupported(locale)
    ...
    for key in otherArgs.keys():
        url += "&%s=%s" % (key, urllib.quote(otherArgs[key]))
    return url
...
def search(search_type, keyword, product_line, type = "heavy", page = None,
           license_key=None, http_proxy = None, locale = None, associate = None, dotFourAPI=None, **keyArgs):
    """search Amazon
    ...
    if dotFourAPI:
        url = buildURLNew(search_type, keyword, product_line, type, page, 
                       license_key, locale, associate,keyArgs)
    else:
...
#     PrettyPrint(xmldoc)
    ...
    usock.close()
    if dotFourAPI: return unmarshal(xmldoc)
...
def searchWishListByEmail(emailAddr,license_key=None, http_proxy=None, locale=None, associate=None):
    results = search("ListSearch", None, None, None, None, license_key, http_proxy, locale, associate, True, Email=emailAddr,ListType='WishList')
    name = results.ListSearchResponse.Lists.List.CustomerName
    wishListId = results.ListSearchResponse.Lists.List.ListId
    return [name,searchByWishlist(wishListId)]

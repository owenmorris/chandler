...
# initialization
filename = "TestCertstoreView.log"
logger = QAUITestAppLib.QALogger(filename, filename[:-4])
...
try:
    ...
    if not app_ns().appbar.pressed(name='ApplicationBarAllButton'):
        logger.ReportFailure("Did not switch to All view as expected")
    
    # 3. confirm that the first certificate in the summary view is selected
    if app_ns().summary.contents.getFirstSelectedItem().who != 'Verisign/RSA Secure Server CA':
        logger.ReportFailure("RSA certificate not first selected certificate in summary list")

    # Get a reference to the detail view that we'll use in later tests
    cert = QAUITestAppLib.UITestItem("Certificate", logger)

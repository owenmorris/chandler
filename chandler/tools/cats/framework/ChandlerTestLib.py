...
def publishSubscribe(testClass):
    ...
    if collection is not None:
        ...
        # find the account
        testClass.logger.startAction('Find morsecode account')
        for account in sharing.CosmoAccount.iterItems(App_ns.itsView):
            if account.displayName == accountName:
                testClass.logger.endAction(True, "(Find morsecode account)")
                break
        else:
            # We couldn't find the morsecode account we created above.
            # raise an error?
            testClass.logger.endAction(False, "(Find morsecode account)")
    ...
    def Check_Object(self, dict):
        ...
        for field,value in dict.iteritems():
            if field == "displayName": # display name checking
                ...
                f = "%s" %pim.mail.MailStamp(self.item).fromAddress
                if not value == f :
                ...
                t = "%s" % pim.mail.MailStamp(self.item).toAddress
                if not value == t :
        ...
        if account is not None:
            ...
            for (key, value) in keys.items():
                ...
                if actualValue != value:
                    self.logger.report(False, name="VerifyValues", comment="Checking %s %s: expected %s, but got %s" % (type, key, value, actualValue))
                    result = False
                else:
    ...
    def DoubleClickInCalView(self, x=100, y=100, gotoTestDate=True):
        ...
        if self.state == "CalendarView":
            ...
            if canvasItem:
                ...
                applicaion = wx.GetApp()
                applicaion.Yield(True)
